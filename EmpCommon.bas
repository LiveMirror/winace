...
'120916 rjk: Changes included in server release 4.3.34. Incorporated the bwork and changes
'            to values related to 100% instead 1% for sector builds.  Incorporate to build
'            sector with all items types not just lcms and hcms.
...
If Len(Trim$(rs.Fields("sdes"))) > 0 And rs.Fields("eff") > 0 _
    ...
    If VersionCheck(4, 3, 34) <> VER_LESS Then
        workcost = 4 * buildavail / rsBuild("bwork")
        If workcost < rs.Fields("eff") Then
            newdes = rs.Fields("des")
            NewEff = rs.Fields("eff") - workcost
        Else
            NewEff = 0
            workcost = rs.Fields("eff")
            newdes = rs.Fields("sdes")
        End If
        buildcost = Int(workcost / 4#)
        buildavail = buildavail - Int((workcost / 4# * rsBuild("bwork")))
    ...
    If Not rsBuild.NoMatch Then
        If VersionCheck(4, 3, 34) <> VER_LESS Then
            If rsBuild.Fields("lcm") > 0 Then
                If (rsBuild.Fields("lcm") * workcost / 100#) > rs.Fields("lcm") Then
                    workcost = Int(rs.Fields("lcm") / (rsBuild.Fields("lcm") / 100#))
                End If
            End If
            If rsBuild.Fields("hcm") > 0 Then
                If (rsBuild.Fields("hcm") * workcost / 100#) > rs.Fields("hcm") Then
                    workcost = Int(rs.Fields("hcm") / (rsBuild.Fields("hcm") / 100#))
                End If
            End If
            If rsBuild.Fields("mil") > 0 Then
                If (rsBuild.Fields("mil") * workcost / 100#) > rs.Fields("mil") Then
                    workcost = Int(rs.Fields("mil") / (rsBuild.Fields("mil") / 100#))
                End If
            End If
            If rsBuild.Fields("gun") > 0 Then
                If (rsBuild.Fields("gun") * workcost / 100#) > rs.Fields("gun") Then
                    workcost = Int(rs.Fields("gun") / (rsBuild.Fields("gun") / 100#))
                End If
            End If
            If rsBuild.Fields("oil") > 0 Then
                If (rsBuild.Fields("oil") * workcost / 100#) > rs.Fields("oil") Then
                    workcost = Int(rs.Fields("oil") / (rsBuild.Fields("oil") / 100#))
                End If
            End If
            If rsBuild.Fields("hcm") > 0 Then
                If (rsBuild.Fields("hcm") * workcost / 100#) > rs.Fields("hcm") Then
            ...
            If rsBuild.Fields("hcm") > 0 Then
                ...
                End If
            End If
            If rsBuild.Fields("pet") > 0 Then
                If (rsBuild.Fields("pet") * workcost / 100#) > rs.Fields("pet") Then
                    workcost = Int(rs.Fields("pet") / (rsBuild.Fields("pet") / 100#))
                End If
            End If
            If rsBuild.Fields("shell") > 0 Then
                If (rsBuild.Fields("shell") * workcost / 100#) > rs.Fields("shell") Then
                    workcost = Int(rs.Fields("shell") / (rsBuild.Fields("shell") / 100#))
                End If
            End If
            If rsBuild.Fields("iron") > 0 Then
                If (rsBuild.Fields("iron") * workcost / 100#) > rs.Fields("iron") Then
                    workcost = Int(rs.Fields("iron") / (rsBuild.Fields("iron") / 100#))
                End If
            End If
            If rsBuild.Fields("dust") > 0 Then
                If (rsBuild.Fields("dust") * workcost / 100#) > rs.Fields("dust") Then
                    workcost = Int(rs.Fields("dust") / (rsBuild.Fields("dust") / 100#))
                End If
            End If
            If rsBuild.Fields("bar") > 0 Then
                If (rsBuild.Fields("bar") * workcost / 100#) > rs.Fields("bar") Then
                    workcost = Int(rs.Fields("bar") / (rsBuild.Fields("bar") / 100#))
                End If
            End If
            If rsBuild.Fields("food") > 0 Then
                If (rsBuild.Fields("food") * workcost / 100#) > rs.Fields("food") Then
                    workcost = Int(rs.Fields("food") / (rsBuild.Fields("food") / 100#))
                End If
            End If
            If rsBuild.Fields("uw") > 0 Then
                If (rsBuild.Fields("uw") * workcost / 100#) > rs.Fields("uw") Then
                    workcost = Int(rs.Fields("uw") / (rsBuild.Fields("uw") / 100#))
                End If
            End If
            workcost = workcost * rsBuild.Fields("bwork")
        Else
            If rsBuild.Fields("lcm") > 0 Then
...
End If
If VersionCheck(4, 3, 34) <> VER_LESS Then
    buildcost = buildcost + (workcost / rsBuild.Fields("bwork") * rsBuild.Fields("cost") / 100#)
    buildavail = (buildavail - workcost) / 100
    NewEff = NewEff + Int(workcost / rsBuild("bwork"))
    maxavail = maxavail - avail
Else
    buildavail = buildavail - workcost
