...
'100215 rjk: Removing SHIP Orders
'160724 rjk: Changes for 4.3.34 server changes
'            Changed ParseXDVersion to bump Revision if running against a modified version
'            Changed value to power in ParseXDItemChr, 4.3.34 server changes.
'            Added XD_SECTOR_CHR_FLAGS for deity flag.
'            Updates to sect-chr, ship-chr, plane-chr, land-chr, nuke-chr fields
'               "c_build", "m_build", "g_build", "p_build", "s_build", "i_build", "d_build", _
'               "b_build", "f_build", "o_build", "u_build", "r_build", use new EmpItemBuild class.
'            Incorporate the removed of the 'c' type in xdump for fleet, wing and army.
'            Updates to ParseXDSectChar  added "bwork"
'            Added melt_denom to XDItemChr.
'            Updated the version field
...
If Left$(strLine, 6) = "XDUMP " Then
    ...
    ElseIf UBound(strParams) = 2 Then
        ...
        If xdType <> XD_UNKNOWN Then
            ...
            Case XD_LEVEL, XD_META_TYPE, XD_MISSIONS, XD_SECTOR_NAVIGATION, XD_SECTOR_CHR_FLAGS, XD_SHIP_CHR_FLAGS
...
Dim cItemBuild As EmpItemBuild

Set cItemBuild = New EmpItemBuild
...
For iIndex = 0 To UBound(strParams)
    ...
    Case "name"
        ...
        cItemBuild.desc = RemoveEscapeSequencesAndQuotes(strParams(iIndex))
    Case "mnem"
    ...
    Case "flg", "value", "uid", "maint", "power"
    Case "flags"
        Dim lFlags As Long
        Dim iFlagIndex As Integer
        
        lFlags = CLng(strParams(iIndex))
        
        Do While lFlags > 0
            If ((lFlags And &H1) > 0) Then
                Select Case estsTable(XD_SECTOR_CHR_FLAGS)(1).Name
                Case "deity"
                    rsSectorType.Fields("deity") = True
                End Select
            Else
                Select Case estsTable(XD_SECTOR_CHR_FLAGS)(1).Name
                Case "deity"
                    rsSectorType.Fields("deity") = False
                End Select
            End If
            lFlags = lFlags \ 2
        Loop
    ...
    Case "mob1"
        ...
        End If
    Case "cost"
        cItemBuild.Cost = strParams(iIndex)
    Case "build"
        cItemBuild.Stat(1) = strParams(iIndex)
    Case "lcms", "l_build"
        cItemBuild.lcm = strParams(iIndex)
    Case "hcms", "h_build"
        cItemBuild.hcm = strParams(iIndex)
    Case "c_build"
        cItemBuild.Civ = strParams(iIndex)
    Case "m_build"
        cItemBuild.Mil = strParams(iIndex)
    Case "g_build"
        cItemBuild.gun = strParams(iIndex)
    Case "p_build"
        cItemBuild.Pet = strParams(iIndex)
    Case "s_build"
        cItemBuild.Shell = strParams(iIndex)
    Case "i_build"
        cItemBuild.Iron = strParams(iIndex)
    Case "d_build"
        cItemBuild.Dust = strParams(iIndex)
    Case "b_build"
        cItemBuild.Bar = strParams(iIndex)
    Case "f_build"
        cItemBuild.Food = strParams(iIndex)
    Case "o_build"
        cItemBuild.Oil = strParams(iIndex)
    Case "u_build"
        cItemBuild.UW = strParams(iIndex)
    Case "r_build"
        cItemBuild.Rad = strParams(iIndex)
    Case "bwork"
        cItemBuild.BWork = strParams(iIndex)
    Case Else
        EmpireError "ParseXDSectorChr", "Field not understood:" + strErrorField, strLine
    End Select
    iHeaderIndex = iHeaderIndex + 1
...
If cItemBuild.Cost >= 0 Then
    cItemBuild.id = strParams(iDesIndex)
    cItemBuild.BuildType = "i"
    cItemBuild.InsertOrUpdateDB
End If
...
For iIndex = 0 To UBound(strParams)
    ...
    Case "wing"
        If estsTable(XD_META_TYPE)(etTable.Column(iHeaderIndex).ColType).Name = "c" Or _
                estsTable(XD_META_TYPE)(etTable.Column(iHeaderIndex).ColType).Name = "s" _
            Then
...
For iIndex = 0 To UBound(strParams)
    ...
    Case "name"
        strParams(iIndex) = RemoveEscapeSequencesAndQuotes(strParams(iIndex))
        iPos = InStr(strParams(iIndex), " ")
        If iPos > 0 Then
            cItemBuild.id = Left$(strParams(iIndex), iPos - 1)
            cItemBuild.desc = LTrim$(Mid$(strParams(iIndex), iPos + 1, Len(strParams(iIndex)) - iPos))
        Else
    ...
    Case "cost"
        cItemBuild.Cost = strParams(iIndex)
    Case "tech"
        cItemBuild.Tech = strParams(iIndex)
    Case "build"
        cItemBuild.Stat(1) = strParams(iIndex)
    Case "lcms", "l_build"
        cItemBuild.lcm = strParams(iIndex)
    ...
    Case "flags"
        ...
        lFlags = CLng(strParams(iIndex))
        If (lFlags And &H1) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "bomber"
            iCapIndex = iCapIndex + 1
        End If
        If (lFlags And &H2) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "tactical"
            iCapIndex = iCapIndex + 1
        End If
        If (lFlags And &H4) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "intercept"
            iCapIndex = iCapIndex + 1
        End If
        If (lFlags And &H8) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "cargo"
            iCapIndex = iCapIndex + 1
        End If
        If (lFlags And &H10) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "VTOL"
            iCapIndex = iCapIndex + 1
        End If
        If (lFlags And &H20) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "missile"
            iCapIndex = iCapIndex + 1
        End If
        If (lFlags And &H40) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "light"
            iCapIndex = iCapIndex + 1
        End If
        If (lFlags And &H80) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "spy"
            iCapIndex = iCapIndex + 1
        End If
        If (lFlags And &H100) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "image"
            iCapIndex = iCapIndex + 1
        End If
        If (lFlags And &H200) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "satellite"
            iCapIndex = iCapIndex + 1
        End If
        If (lFlags And &H400) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "stealth"
            iCapIndex = iCapIndex + 1
        End If
        If (lFlags And &H800) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "SDI"
            iCapIndex = iCapIndex + 1
        End If
        If (lFlags And &H1000) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "half-stealth"
            iCapIndex = iCapIndex + 1
        End If
        If (lFlags And &H2000) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "x-light"
            iCapIndex = iCapIndex + 1
        End If
        If (lFlags And &H4000) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "helo"
            iCapIndex = iCapIndex + 1
        End If
        If (lFlags And &H8000&) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "ASW"
            iCapIndex = iCapIndex + 1
        End If
        If (lFlags And &H10000) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "para"
            iCapIndex = iCapIndex + 1
        End If
        If (lFlags And &H20000) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "escort"
            iCapIndex = iCapIndex + 1
        End If
        If (lFlags And &H40000) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "mine"
            iCapIndex = iCapIndex + 1
        End If
        If (lFlags And &H80000) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "sweep"
            iCapIndex = iCapIndex + 1
        End If
        If (lFlags And &H100000) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "marine"
            iCapIndex = iCapIndex + 1
        End If
        cItemBuild.Cap(iCapIndex) = ""
    Case "acc"
        cItemBuild.Base_Acc = strParams(iIndex)
        If CheckIndex(iTechIndex, "ParseXDPlaneChr", "tech") Then
            cItemBuild.Stat(1) = Fix(ComputePlaneAccuracy(CInt(strParams(iIndex)), CInt(strParams(iTechIndex)), GetTechLevelForShow()))
        End If
    Case "load"
        cItemBuild.Base_Load = strParams(iIndex)
        If CheckIndex(iTechIndex, "ParseXDPlaneChr", "tech") Then
            cItemBuild.Stat(2) = Fix(ComputePlaneLoad(CInt(strParams(iIndex)), CInt(strParams(iTechIndex)), GetTechLevelForShow()))
        End If
    Case "att"
        cItemBuild.Base_Att = strParams(iIndex)
        If CheckIndex(iTechIndex, "ParseXDPlaneChr", "tech") Then
            cItemBuild.Stat(3) = Fix(ComputePlaneAttDef(CInt(strParams(iIndex)), CInt(strParams(iTechIndex)), GetTechLevelForShow()))
        End If
    Case "def"
        cItemBuild.Base_Def = strParams(iIndex)
        If CheckIndex(iTechIndex, "ParseXDPlaneChr", "tech") Then
            cItemBuild.Stat(4) = Fix(ComputePlaneAttDef(CInt(strParams(iIndex)), CInt(strParams(iTechIndex)), GetTechLevelForShow()))
        End If
    Case "range"
        cItemBuild.Base_Spd = CInt(strParams(iIndex))
        If CheckIndex(iTechIndex, "ParseXDPlaneChr", "tech") Then
            cItemBuild.Stat(5) = Fix(ComputePlaneRange(CInt(strParams(iIndex)), CInt(strParams(iTechIndex)), GetTechLevelForShow()))
        End If
    Case "fuel"
        cItemBuild.Stat(6) = strParams(iIndex)
    Case "stealth"
        cItemBuild.Stat(7) = strParams(iIndex)
    Case "type"
        cItemBuild.ChrIndex = strParams(iIndex)
    Case "bwork"
        cItemBuild.BWork = strParams(iIndex)
    Case Else
...
iOpyIndex = etTable.GetZeroIndex("opy")
iRadiusIndex = etTable.GetZeroIndex("radius")
If VersionCheck(4, 3, 32) = VER_LESS Then
...
Else
    If iOwnerIndex = -1 Or iIDIndex = -1 Or iOpxIndex = -1 Or iOpyIndex = -1 Or _
        iRadiusIndex = -1 Or iXlocIndex = -11 Or iYlocIndex = -1 Or iTypeIndex = -1 _
        Then
        EmpireError "ParseXDShip", _
            "One of the following fields is missing: owner, id, xloc, yloc, opx, opy, rad or type", _
             CStr(iIDIndex)
        rsShip.Index = hldShipIndex
        Exit Sub
    End If
...
For iIndex = 0 To UBound(strParams)
    ...
    Case "civil"
        If Len(strParams(iIndex)) > 0 And strParams(iIndex) > 0 Then
            cItemBuild.Cap(iCapIndex) = strParams(iIndex) + "c"
            iCapIndex = iCapIndex + 1
        End If
    Case "milit"
        If Len(strParams(iIndex)) > 0 And strParams(iIndex) > 0 Then
            cItemBuild.Cap(iCapIndex) = strParams(iIndex) + "m"
            iCapIndex = iCapIndex + 1
        End If
    Case "shell"
        If Len(strParams(iIndex)) > 0 And strParams(iIndex) > 0 Then
            cItemBuild.Cap(iCapIndex) = strParams(iIndex) + "s"
            iCapIndex = iCapIndex + 1
        End If
    Case "gun"
        If Len(strParams(iIndex)) > 0 And strParams(iIndex) > 0 Then
            cItemBuild.Cap(iCapIndex) = strParams(iIndex) + "g"
            iCapIndex = iCapIndex + 1
        End If
    Case "petrol"
        If Len(strParams(iIndex)) > 0 And strParams(iIndex) > 0 Then
            cItemBuild.Cap(iCapIndex) = strParams(iIndex) + "p"
            iCapIndex = iCapIndex + 1
        End If
    Case "iron"
        If Len(strParams(iIndex)) > 0 And strParams(iIndex) > 0 Then
            cItemBuild.Cap(iCapIndex) = strParams(iIndex) + "i"
            iCapIndex = iCapIndex + 1
        End If
    Case "dust"
        If Len(strParams(iIndex)) > 0 And strParams(iIndex) > 0 Then
            cItemBuild.Cap(iCapIndex) = strParams(iIndex) + "d"
            iCapIndex = iCapIndex + 1
        End If
    Case "bar"
        If Len(strParams(iIndex)) > 0 And strParams(iIndex) > 0 Then
            cItemBuild.Cap(iCapIndex) = strParams(iIndex) + "b"
            iCapIndex = iCapIndex + 1
        End If
    Case "food"
        If Len(strParams(iIndex)) > 0 And strParams(iIndex) > 0 Then
            cItemBuild.Cap(iCapIndex) = strParams(iIndex) + "f"
            iCapIndex = iCapIndex + 1
        End If
    Case "oil"
        If Len(strParams(iIndex)) > 0 And strParams(iIndex) > 0 Then
            cItemBuild.Cap(iCapIndex) = strParams(iIndex) + "o"
            iCapIndex = iCapIndex + 1
        End If
    Case "lcm"
        If Len(strParams(iIndex)) > 0 And strParams(iIndex) > 0 Then
            cItemBuild.Cap(iCapIndex) = strParams(iIndex) + "l"
            iCapIndex = iCapIndex + 1
        End If
    Case "hcm"
        If Len(strParams(iIndex)) > 0 And strParams(iIndex) > 0 Then
            cItemBuild.Cap(iCapIndex) = strParams(iIndex) + "h"
            iCapIndex = iCapIndex + 1
        End If
    Case "uw"
        If Len(strParams(iIndex)) > 0 And strParams(iIndex) > 0 Then
            cItemBuild.Cap(iCapIndex) = strParams(iIndex) + "u"
            iCapIndex = iCapIndex + 1
        End If
    Case "rad"
        If Len(strParams(iIndex)) > 0 And strParams(iIndex) > 0 Then
            cItemBuild.Cap(iCapIndex) = strParams(iIndex) + "r"
            iCapIndex = iCapIndex + 1
        End If
    Case "flags"
        If (strParams(iIndex) And &H1) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "fish"
            iCapIndex = iCapIndex + 1
        End If
        If (strParams(iIndex) And &H2) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "torp"
            iCapIndex = iCapIndex + 1
        End If
        If (strParams(iIndex) And &H4) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "dchrg"
            iCapIndex = iCapIndex + 1
        End If
        If (strParams(iIndex) And &H8) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "plane"
            iCapIndex = iCapIndex + 1
        End If
        If (strParams(iIndex) And &H10) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "miss"
            iCapIndex = iCapIndex + 1
        End If
        If (strParams(iIndex) And &H20) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "oil"
            iCapIndex = iCapIndex + 1
        End If
        If (strParams(iIndex) And &H40) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "sonar"
            iCapIndex = iCapIndex + 1
        End If
        ...
        If (strParams(iIndex) And &H800) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "land"
            iCapIndex = iCapIndex + 1
        End If
        If (strParams(iIndex) And &H1000) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "sub-torp"
            iCapIndex = iCapIndex + 1
        End If
        If (strParams(iIndex) And &H2000) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "trade"
            iCapIndex = iCapIndex + 1
        End If
        If (strParams(iIndex) And &H4000) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "semi-land"
            iCapIndex = iCapIndex + 1
        End If
...
'        End If
        If (strParams(iIndex) And &H20000) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "oiler"
            iCapIndex = iCapIndex + 1
        End If
        If (strParams(iIndex) And &H40000) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "supply"
            iCapIndex = iCapIndex + 1
        End If
        If (strParams(iIndex) And &H80000) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "canal"
            iCapIndex = iCapIndex + 1
        End If
        If (strParams(iIndex) And &H100000) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "anti-missile"
            iCapIndex = iCapIndex + 1
        End If
    Case "armor"
        cItemBuild.Base_Def = strParams(iIndex)
        If CheckIndex(iTechIndex, "ParseXDShipChr", "tech") Then
            cItemBuild.Stat(1) = Fix(ComputeShipDefense(CInt(strParams(iIndex)), CInt(strParams(iTechIndex)), GetTechLevelForShow()))
        End If
    Case "speed"
        cItemBuild.Base_Spd = strParams(iIndex)
        If CheckIndex(iTechIndex, "ParseXDShipChr", "tech") Then
            cItemBuild.Stat(2) = Fix(ComputeShipSpeed(CInt(strParams(iIndex)), CInt(strParams(iTechIndex)), GetTechLevelForShow()))
        End If
    Case "visib"
        cItemBuild.Base_Vul = strParams(iIndex)
        If CheckIndex(iTechIndex, "ParseXDShipChr", "tech") Then
            cItemBuild.Stat(3) = Fix(ComputeShipVisibility(CInt(strParams(iIndex)), CInt(strParams(iTechIndex)), GetTechLevelForShow()))
        End If
    Case "vrnge"
            cItemBuild.Stat(4) = strParams(iIndex)
    Case "frnge"
        cItemBuild.Base_Frnge = strParams(iIndex)
        If CheckIndex(iTechIndex, "ParseXDShipChr", "tech") Then
            cItemBuild.Stat(5) = Fix(ComputeShipRange(CInt(strParams(iIndex)), CInt(strParams(iTechIndex)), GetTechLevelForShow()))
        End If
    Case "glim"
        cItemBuild.Base_Load = strParams(iIndex)
        If CheckIndex(iTechIndex, "ParseXDShipChr", "tech") Then
            cItemBuild.Stat(6) = Fix(ComputeShipFiringRange(CInt(strParams(iIndex)), CInt(strParams(iTechIndex)), GetTechLevelForShow()))
        End If
    Case "nland"
        cItemBuild.Stat(7) = strParams(iIndex)
    Case "nplanes"
        cItemBuild.Stat(8) = strParams(iIndex)
    Case "nchoppers"
        cItemBuild.Stat(9) = strParams(iIndex)
    Case "nxlight"
        cItemBuild.Stat(10) = strParams(iIndex)
    Case "fuelc"
        cItemBuild.Stat(11) = strParams(iIndex)
    Case "fuelu"
        cItemBuild.Stat(12) = strParams(iIndex)
    Case "type"
        cItemBuild.ChrIndex = strParams(iIndex)
    Case "bwork"
        'do nothing for now.
...
'            EmpireError "ParseXDLand", "Land Characteristics not found:" + rsLand.Fields("type"), strLine
    ...
    Case "off"
        ...
        End If
    Case "c_build", "m_build", "g_build", "p_build", "s_build", "i_build", "d_build", _
               "b_build", "f_build", "o_build", "u_build", "r_build", "bwork"
        'do nothing for now.
    Case Else
        EmpireError "ParseXDLand", "Field not understood:" + strErrorField, strLine
    End Select
...
For iIndex = 0 To UBound(strParams)
    ...
    Case "name"
        ...
        If iPos > 0 Then
            cItemBuild.id = Left$(strParams(iIndex), iPos - 1)
            cItemBuild.desc = LTrim$(Right$(strParams(iIndex), Len(strParams(iIndex)) - iPos))
        Else
    ...
    Case "flags"
        If (strParams(iIndex) And &H1) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "xlight"
            iCapIndex = iCapIndex + 1
        End If
        If (strParams(iIndex) And &H2) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "engineer"
            iCapIndex = iCapIndex + 1
        End If
        If (strParams(iIndex) And &H4) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "supply"
        ...
        End If
        If (strParams(iIndex) And &H8) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "security"
            iCapIndex = iCapIndex + 1
        End If
        If (strParams(iIndex) And &H10) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "light"
        ...
        End If
        If (strParams(iIndex) And &H40) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "recon"
            iCapIndex = iCapIndex + 1
        End If
        If (strParams(iIndex) And &H80) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "radar"
            iCapIndex = iCapIndex + 1
        End If
        If (strParams(iIndex) And &H100) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "assault"
            iCapIndex = iCapIndex + 1
        End If
        If (strParams(iIndex) And &H200) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "flak"
            iCapIndex = iCapIndex + 1
        End If
        If (strParams(iIndex) And &H400) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "spy"
        ...
        End If
        If (strParams(iIndex) And &H800) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "train"
            iCapIndex = iCapIndex + 1
        End If
        If (strParams(iIndex) And &H1000) <> 0 Then
            cItemBuild.Cap(iCapIndex) = "heavy"
            iCapIndex = iCapIndex + 1
        End If
    Case "att"
        cItemBuild.Base_Att = Val(strParams(iIndex))
        If CheckIndex(iTechIndex, "ParseXDLandChr", "tech") Then
            cItemBuild.Stat(1) = Round(ComputeLandAttDef(Val(strParams(iIndex)), CInt(strParams(iTechIndex)), GetTechLevelForShow()), 1)
        End If
    Case "def"
        cItemBuild.Base_Def = Val(strParams(iIndex))
        If CheckIndex(iTechIndex, "ParseXDLandChr", "tech") Then
            cItemBuild.Stat(2) = Round(ComputeLandAttDef(Val(strParams(iIndex)), CInt(strParams(iTechIndex)), GetTechLevelForShow()), 1)
        End If
    Case "vul"
        cItemBuild.Base_Vul = strParams(iIndex)
        If CheckIndex(iTechIndex, "ParseXDLandChr", "tech") Then
            cItemBuild.Stat(3) = Fix(ComputeLandVulerability(CInt(strParams(iIndex)), CInt(strParams(iTechIndex)), GetTechLevelForShow()))
        End If
    Case "spd"
        cItemBuild.Base_Spd = strParams(iIndex)
        If CheckIndex(iTechIndex, "ParseXDLandChr", "tech") Then
            cItemBuild.Stat(4) = Fix(ComputeLandSpeed(CInt(strParams(iIndex)), CInt(strParams(iTechIndex)), GetTechLevelForShow()))
        End If
    Case "vis"
        cItemBuild.Stat(5) = strParams(iIndex)
    Case "spy"
        cItemBuild.Stat(6) = strParams(iIndex)
    Case "rmax"
        cItemBuild.Stat(7) = strParams(iIndex)
    Case "frg"
        cItemBuild.Base_Frnge = strParams(iIndex)
        If CheckIndex(iTechIndex, "ParseXDLandChr", "tech") Then
            cItemBuild.Stat(8) = Fix(ComputeLandFiringRange(CInt(strParams(iIndex)), CInt(strParams(iTechIndex)), GetTechLevelForShow()))
        End If
    Case "acc"
        cItemBuild.Base_Acc = strParams(iIndex)
        If CheckIndex(iTechIndex, "ParseXDLandChr", "tech") Then
            cItemBuild.Stat(9) = Fix(ComputeLandAccuracy(CInt(strParams(iIndex)), CInt(strParams(iTechIndex)), GetTechLevelForShow()))
        End If
    Case "dam"
        cItemBuild.Base_Dam = strParams(iIndex)
        If CheckIndex(iTechIndex, "ParseXDLandChr", "tech") Then
            cItemBuild.Stat(10) = Fix(ComputeLandDamage(CInt(strParams(iIndex)), CInt(strParams(iTechIndex)), GetTechLevelForShow()))
        End If
    Case "ammo"
        cItemBuild.Base_Load = strParams(iIndex)
        If CheckIndex(iTechIndex, "ParseXDLandChr", "tech") And _
           CheckIndex(iDamageIndex, "ParseXDLandChr", "damage") Then
            cItemBuild.Stat(11) = Fix(ComputeLandAmmo(CInt(strParams(iIndex)), CInt(strParams(iDamageIndex)), CInt(strParams(iTechIndex)), GetTechLevelForShow()))
        End If
    Case "aaf"
        cItemBuild.Base_Aaf = strParams(iIndex)
        If CheckIndex(iTechIndex, "ParseXDLandChr", "tech") Then
            cItemBuild.Stat(12) = Fix(ComputeLandAAF(CInt(strParams(iIndex)), CInt(strParams(iTechIndex)), GetTechLevelForShow()))
        End If
    Case "fuelc"
        cItemBuild.Stat(13) = strParams(iIndex)
    Case "fuelu"
        cItemBuild.Stat(14) = strParams(iIndex)
    Case "nxlight"
        cItemBuild.Stat(15) = strParams(iIndex)
    Case "nland"
        cItemBuild.Stat(16) = strParams(iIndex)
    Case "type"
        cItemBuild.ChrIndex = strParams(iIndex)
    Case "bwork"
        cItemBuild.BWork = strParams(iIndex)
...
cItemBuild.avail = CalculateAvail(cItemBuild.lcm, cItemBuild.hcm)
cItemBuild.BuildType = "l"
cItemBuild.InsertOrUpdateDB
...
For iIndex = 0 To UBound(strParams)
    ...
    Case "name"
        ...
        iPos = InStr(strParams(iIndex), " ")
        If iPos > 0 Then
            cItemBuild.id = Left$(Left$(strParams(iIndex), iPos - 1), 5)
            cItemBuild.desc = LTrim$(Mid$(strParams(iIndex), iPos + 1, Len(strParams(iIndex)) - iPos))
        Else
            EmpireError "ParseXDNukeChr", "Incorrect format of name", strLine
        End If
    Case "avail"
        cItemBuild.avail = strParams(iIndex)
    Case "cost"
        cItemBuild.Cost = strParams(iIndex)
    Case "tech"
        cItemBuild.Tech = strParams(iIndex)
    ...
    Case "blast"
        cItemBuild.Stat(1) = strParams(iIndex)
    Case "dam"
        cItemBuild.Stat(2) = strParams(iIndex)
    Case "weight"
        cItemBuild.Stat(3) = strParams(iIndex)
    Case "tech"
        cItemBuild.Tech = strParams(iIndex)
        cItemBuild.Stat(4) = strParams(iIndex)
        ...
        If sDRNukeConst > 0# Then
            cItemBuild.Stat(5) = Int((strParams(iIndex) * sDRNukeConst)) + 1
        Else
            cItemBuild.Stat(5) = 0
        End If
    Case "type"
...
For iIndex = 0 To UBound(strParams)
    ...
    Case "pkg"
        ...
        iIndex = iIndex + 4
    Case "melt_denom"
        eiChr.MeltDenomination = strParams(iIndex)
    Case Else
...
'                    rsBuild.CancelUpdate
        ...
        End If
    Case Else
        EmpireError "ParseXDInfraChr", "Field not understood:" + strErrorField, strLine
    End Select
    iHeaderIndex = iHeaderIndex + 1
Next iIndex
    ...
    Case "version"
        ...
        If Len(strParams(iIndex)) > 1 And iPos > 0 Then
            ...
            If UBound(strVersions) = 2 Then
                ...
                iPos = InStr(strVersions(2), "-")
                
                If iPos > 0 Then
                    rsVersion.Fields("Revision") = Left(strVersions(2), iPos - 1)
                Else
                    rsVersion.Fields("Revision") = strVersions(2)
                End If
'RJK Temporary Code
            ElseIf UBound(strVersions) = 3 Then
                rsVersion.Fields("Major") = strVersions(0)
                rsVersion.Fields("Minor") = strVersions(1)
                rsVersion.Fields("Revision") = strVersions(2) + 1
            Else
...
Case "sect-chr-flags"
    ParseXDTableName = XD_SECTOR_CHR_FLAGS
Case "sect-chr"
...
If VersionCheck(4, 3, 34) <> VER_LESS Then
    frmEmpCmd.SubmitEmpireCommand "xdump meta sect-chr-flags", False
End If
If VersionCheck(4, 3, 10) <> VER_LESS Then
...
If VersionCheck(4, 3, 34) <> VER_LESS Then
    frmEmpCmd.SubmitEmpireCommand "xdump sect-chr-flags *", False
End If
