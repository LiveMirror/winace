...
'110916 rjk: Changes for server release 4.3.34, new version format, including development releases
'270817 rjk: Changes for server release 4.4.0, new version format, including release candidates.

'Parse the results of a coastwatch command
Public Function ParseCoastWatch(strLook As String) As Variant
...
ReDim SecList(0 To ES_NUM)
For n = 1 To ES_NUM
    SecList(n) = "???"
Next n

If Len(strLook) = 0 Then
    ParseCoastWatch = SecList()
    Exit Function
End If
...
If n > 0 Then
    ...
    t2 = n2 + 1
    owner = CInt(Trim$(Mid$(strLook, n + 2, n2 - n - 2)))
End If
...
Public Function ParseSonar(strSonar As String) As Variant
...
Public Function ParseForNumber(strSample As String) As Integer
...
End Function
Public Sub ParseVersion(strLine As String, ByRef rs As Recordset)
Dim n As Integer
Dim n2 As Integer
Dim n3 As Integer
Dim n4 As Integer
...
If n = 1 Or n2 = 1 Then
    ...
    If n > 0 Then
        ...
        If n2 > 0 Then
            rs.Fields(var) = CInt(Trim$(Mid$(strLine, n2 + 1, n - n2 - 1)))
            var = "Minor"
            n2 = InStr(n + 1, strLine, ".")
            If n2 > 0 Then
                rs.Fields(var) = CInt(Trim$(Mid$(strLine, n + 1, n2 - n - 1)))
                var = "Revision"
                n3 = InStr(n2 + 1, strLine, " ")
                If n3 > 0 Then
                    rs.Fields(var) = CInt(Trim$(Mid$(strLine, n2 + 1, n3 - n2 - 1)))
                Else
                    n4 = InStr(n2 + 1, strLine, ".")
                    If n4 > 0 Then
                        rs.Fields(var) = CInt(Trim$(Mid$(strLine, n2 + 1, n4 - (n2 + 1)))) + 1
                    Else
                        n4 = InStr(Mid$(strLine, n2 + 1, Len(strLine) - n2), "-")
                        If n4 > 0 Then
                            rs.Fields(var) = CInt(Trim$(Left(Mid$(strLine, n2 + 1, Len(strLine) - n2), n4 - 1)))
                        Else
                            rs.Fields(var) = CInt(Trim$(Mid$(strLine, n2 + 1, Len(strLine) - n2)))
                        End If
                    End If
                End If
            End If
...
If n > 0 Then
    ...
    If n > 0 And n2 > 0 And n3 > 0 Then
        var = "World X"
        rs.Fields(var) = CInt(Trim$(Mid$(strLine, n + 3, n2 - n - 3)))
        var = "World Y"
        rs.Fields(var) = CInt(Trim$(Mid$(strLine, n2 + 3, n3 - n2 - 3)))
    End If
    '300304 rjk: Ensure it is zero, if it is changed to 0 on the server, it will be missing from the version report
    rs.Fields("Rollover Avail") = 0
End If
...
If n > 0 Then
    var = "Time Zone Adj"
    rs.Fields(var) = DateDiff("n", Now, EmpireTimeString("    " + Trim$(Mid$(strLine, 24, 15)) + " " + str(Year(Now))))
End If
...
If n > 0 Then
    ...
    n = InStr(n2, strLine, " ")
    If n > 0 Then
        rs.Fields(var) = CInt(Trim$(Mid$(strLine, n2, n - n2)))
    End If
End If
...
If n > 0 Then
    ...
    n = InStr(n2, strLine, " ")
    If n > 0 Then
        rs.Fields(var) = Val(Trim$(Mid$(strLine, n2, n - n2)))
    End If
End If
...
If n > 0 Then
    ...
    If n > 0 And n2 > 0 Then
        var = "Civ Taxes"
        rs.Fields(var) = Val(Trim$(Mid$(strLine, n + 1, n2 - n)))
...
If n > 0 Then
    ...
    If n > 0 And n2 > 0 Then
        ...
        If n > 0 And n2 > 0 Then
            var = "Reserve Salary"
            rs.Fields(var) = Val(Trim$(Mid$(strLine, n + 1, n2 - n - 1)))
        End If
    End If
End If
...
If n > 0 Then
    var = "Rollover Avail"
    rs.Fields(var) = Val(Trim$(Mid$(strLine, 7, n - 7)))
End If
...
If n > 0 Then
    var = "Tech Decay Rate"
    rs.Fields(var) = Val(Trim$(StringBetween(strLine, "decline", "%")))
    var = "Tech Decay Time"
    rs.Fields(var) = Val(Trim$(StringBetween(strLine, "every", "time units")))
End If
...
If n > 0 Then
    var = "Fire range"
    n2 = 27
    rs.Fields(var) = Val(Trim$(Mid$(strLine, n2)))
End If
...
If n > 0 Then
    ...
    var = "Max mob - Sector"
    rs.Fields(var) = CInt(Trim$(strTokens(2)))
    var = "Max mob - Ship"
    rs.Fields(var) = CInt(Trim$(strTokens(3)))
    var = "Max mob - Plane"
    rs.Fields(var) = CInt(Trim$(strTokens(4)))
    var = "Max mob - Land"
    rs.Fields(var) = CInt(Trim$(strTokens(5)))
End If
...
If n > 0 Then
    ...
    End If
    var = "Eff gain - Ship"
    rs.Fields(var) = CInt(Trim$(strTokens(6)))
    var = "Eff gain - Plane"
    rs.Fields(var) = CInt(Trim$(strTokens(7)))
    var = "Eff gain - Land"
    rs.Fields(var) = CInt(Trim$(strTokens(8)))
End If
...
End Sub
Public Sub ParseNation(strLine As String)
Dim n As Integer
Dim n2 As Integer
' Dim n3 As Integer    8/2003 efj  removed
...
If n > 0 Then
    ...
    CountryNumber = Int(StringBetween(strLine, "(#", ")"))
...
Public Sub ParseDumpString(strLine As String, strTokens() As String, Optional Deityflag As Boolean)

'if this is a deity dump, include special handling for country code
If IsMissing(Deityflag) Then
    Deityflag = False
End If
...
Public Function ParseLook(strLook As String) As Variant
Dim SecList() As String
Dim n As Integer
Dim n2 As Integer
Dim n3 As Integer
...
If n > 0 Then
    n2 = InStr(n + 2, strLook, ")")
    t2 = n2 + 1
    owner = CInt(Mid$(strLook, n + 2, n2 - n - 2))
End If
...
Public Function ParseAttack(strLook As String) As Variant
Dim SecList() As String
Dim n As Integer
...
'get the mil
Mil = Val(Trim$(StringBetween(strLook, "approximately", "military")))
...
'Sector efficency
SecList(ES_EFFICIENCY) = Val(Trim$(StringBetween(strLook, "is a", "%")))
...
Public Function ParseReconReport(strLine As String) As Variant
...
If Mid$(strLine, 5, 1) = "," Then
    ...
    If SecList(ES_OWNER) = CountryNumber Then
...
ElseIf Right$(Trim$(strLine), 1) = "%" And Right$(Trim$(strLine), 2) <> " %" And _
       InStr(strLine, "aborted") = 0 Then
    ...
    'Skip if its on our country
    If SecList(EU_OWNER) = CountryNumber Then
        SecList(0) = vbNullString
    Else
        'get the class (ship, plane, land)
...
Public Function ParseSpyReport(strLine As String) As Variant
...
If InStr(strLine, "unit in") > 0 Then
    ...
    If n > 0 Then
        n2 = InStr(n + 1, strLine, ")")
        owner = Nations.NationNumber(Mid$(strLine, n + 1, n2 - n - 1))
    End If
...
Public Sub ParseShow(strLine As String, tokenstart As Integer, UnitType As String, ReportType As String, LineNumber As Integer)
...
'Parse results of show build string to update database
Public Sub ParseShowBuild(strLine As String, tokenstart As Integer, UnitClass As String, ReportType As String)
Dim j As Integer
Dim k As Integer
Dim n As Integer
Dim fstart As Integer
...
If rsBuild.NoMatch Then
    ...
    For j = 0 To rsBuild.Fields.Count - 1
        If rsBuild.Fields(j).Type = dbText Then
...
If ReportType = "b" Then
    rsBuild.Fields("lcm").Value = CInt(strTokens(0))
    n = 3
    rsBuild.Fields("hcm").Value = CInt(strTokens(1))
    If UnitClass = "s" Then
        n = 4
        rsBuild.Fields("avail").Value = CInt(strTokens(2))
        n = 5
        rsBuild.Fields("tech").Value = CInt(strTokens(3))
        n = 6
        rsBuild.Fields("cost").Value = CLng(Mid$(strTokens(4), 2))
    ElseIf UnitClass = "n" Then
        n = 4 '092503 rjk: switched to 4 from 5 to be consistent
        rsBuild.Fields("oil").Value = CInt(strTokens(2))
        n = 5 '092503 rjk: switched to 5 from 6 to be consistent
        rsBuild.Fields("rad").Value = CInt(strTokens(3))
        n = 6 '092503 rjk: switched to 6 from 7 to be consistent
        rsBuild.Fields("avail").Value = CInt(strTokens(4))
        n = 7 '092503 rjk: switched to 7 from 8 to be consistent
        rsBuild.Fields("tech").Value = CInt(strTokens(5))
        n = 8 '092503 rjk: switched to 8 from 9 to be consistent
        'check for a stand alone '$' on nukes
        If strTokens(7) = "$" Then
            rsBuild.Fields("stat5").Value = CInt(strTokens(6)) 'Research
            rsBuild.Fields("cost").Value = CLng(strTokens(8))
        Else
            rsBuild.Fields("stat5").Value = 0 'Research
            rsBuild.Fields("cost").Value = CLng(strTokens(7))
        End If
    Else
        n = 4
        If UnitClass = "p" Then
            rsBuild.Fields("mil").Value = CInt(strTokens(2))
        Else
            rsBuild.Fields("gun").Value = CInt(strTokens(2))
        End If
        
        n = 5
        rsBuild.Fields("avail").Value = CInt(strTokens(3))
        n = 6
        rsBuild.Fields("tech").Value = CInt(strTokens(4))
        n = 7
        rsBuild.Fields("cost").Value = CLng(Mid$(strTokens(5), 2))
    End If
ElseIf UnitClass = "n" Then
    ...
    If UBound(strTokens) >= 5 Then
        n = 9
        rsBuild.Fields("stat1").Value = CInt(strTokens(0))
        n = 10
        rsBuild.Fields("stat2").Value = CInt(strTokens(1))
        n = 11
        rsBuild.Fields("stat3").Value = CInt(strTokens(2))
        n = 12
        rsBuild.Fields("stat4").Value = CInt(strTokens(3))
        'skip dollar sign
        If strTokens(4) <> "$" Then
            n = 13
            rsBuild.Fields("stat5").ValidateOnSet = CInt(strTokens(4))
            n = 15
            k = 6
        Else
            n = 14
        ...
        rsBuild.Fields("cost").Value = CLng(strTokens(k))
    ...
    For j = LBound(strTokens) To UBound(strTokens)
        If n < 32 Then
            ...
            ElseIf UnitClass = "p" And (InStr(strTokens(j), "%") > 0) Then
                ...
                rsBuild.Fields(n).Value = CInt(Left$(strTokens(j), Len(strTokens(j)) - 1))
            Else
...
'Parse results of show build string to update database
Public Sub ParseShowSector(strLine As String, tokenstart As Integer, ReportType As String)
Dim j As Integer
Dim n As Integer
Dim fstart As Integer
Dim strSpace As String
...
If rsSectorType.NoMatch Then
    ...
    For j = 0 To rsSectorType.Fields.Count - 1
        If rsSectorType.Fields(j).Type = dbText Then
...
Select Case ReportType
    ...
    Else
        For n = 0 To 8
            ...
            Else
                rsSectorType.Fields(n + 2).Value = CInt(strTokens(n))
            End If
        Next n
    End If
    ...
    If n > 0 Then
        rsSectorType.Fields("product") = Trim$(Mid$(strLine, n, 7))
    End If
    ...
    If n > 0 Then
        ...
        If Len(strTemp) > 0 Then
            rsSectorType.Fields("use1s") = strTemp
        Else
            rsSectorType.Fields("use1s") = vbNullString
        End If
        strTemp = Trim$(Mid$(strLine, n, 2))
        If Len(strTemp) > 0 Then
            rsSectorType.Fields("use1n") = CInt(strTemp)
        Else
            rsSectorType.Fields("use1n") = 0
        End If
    End If
    ...
    If n > 0 Then
        strTemp = Trim$(Mid$(strLine, n + 3, 1))
        If Len(strTemp) > 0 Then
            rsSectorType.Fields("use2s") = strTemp
        Else
            rsSectorType.Fields("use2s") = vbNullString
        End If
        strTemp = Trim$(Mid$(strLine, n, 2))
        If Len(strTemp) > 0 Then
            rsSectorType.Fields("use2n") = CInt(strTemp)
        Else
            rsSectorType.Fields("use2n") = 0
        End If
    End If
    ...
    If n > 0 Then
        strTemp = Trim$(Mid$(strLine, n + 3, 1))
        If Len(strTemp) > 0 Then
            rsSectorType.Fields("use3s") = strTemp
        Else
            rsSectorType.Fields("use3s") = vbNullString
        End If
        strTemp = Trim$(Mid$(strLine, n, 2))
        If Len(strTemp) > 0 Then
            rsSectorType.Fields("use3n") = CInt(strTemp)
        Else
            rsSectorType.Fields("use3n") = 0
        End If
    End If

    n = InStr(tokenstart, strHeader, "level")
    If n > 0 Then
        rsSectorType.Fields("level") = Trim$(Mid$(strLine, n, 5))
    End If
    
    n = InStr(tokenstart, strHeader, "min")
    If n > 0 Then
        rsSectorType.Fields("min") = CInt(Trim$(Mid$(strLine, n, 3)))
    End If
    
    n = InStr(tokenstart, strHeader, "lag")
    If n > 0 Then
        rsSectorType.Fields("lag") = CInt(Trim$(Mid$(strLine, n, 3)))
    End If
    
    n = InStr(tokenstart, strHeader, "eff%")
    If n > 0 Then
        rsSectorType.Fields("eff") = CInt(Trim$(Mid$(strLine, n, 4)))
    End If
    
    n = InStr(tokenstart, strHeader, "$$$")
    If n > 0 Then
        rsSectorType.Fields("cost") = CInt(Trim$(Mid$(strLine, n, 4)))
    End If
    
    n = InStr(tokenstart, strHeader, "dep")
    If n > 0 Then
        rsSectorType.Fields("dep") = CInt(Trim$(Mid$(strLine, n, 3)))
    End If
    
    n = InStr(tokenstart, strHeader, " c ")
    If n > 0 Then
        rsSectorType.Fields("pcode") = Trim$(Mid$(strLine, n, 3))
    End If
    If Len(Trim$(rsSectorType.Fields("pcode"))) > 0 Then
        Dim eiItem As EmpItem
        
        Set eiItem = Items.FindByLetter(rsSectorType.Fields("pcode"))
        If IsObject(eiItem) Then
            eiItem.ProductName = rsSectorType.Fields("product")
            eiItem.SaveItem
        End If
    End If
End Select
...
'Parse results of show build string to update database
Private Sub ParseShowItem(strLine As String, Line As Integer)
...
If rsItems.NoMatch Then
    ...
    For fldIndex = 0 To rsItems.Fields.Count - 1
        If rsItems.Fields(fldIndex).Type = dbText Then
...
If n > 0 Then
    rsItems.Fields("value") = Val(Trim$(Mid$(strLine, n, 5)))
End If

n = InStr(strHeader, "sell")
If n > 0 Then
    If InStr(Mid$(strLine, n, 4), "yes") > 0 Then
        rsItems.Fields("sell") = True
    Else
        rsItems.Fields("sell") = False
    End If
End If

n = InStr(strHeader, "lbs")
If n > 0 Then
    rsItems.Fields("lbs") = Val(Trim$(Mid$(strLine, n, 3)))
End If

n = InStr(strHeader2, "rg")
If n > 0 Then
    rsItems.Fields("pack_rg") = Val(Trim$(Mid$(strLine, n, 2)))
End If

n = InStr(strHeader2, "wh")
If n > 0 Then
    rsItems.Fields("pack_wh") = Val(Trim$(Mid$(strLine, n, 2)))
End If

n = InStr(strHeader2, "ur")
If n > 0 Then
    rsItems.Fields("pack_ur") = Val(Trim$(Mid$(strLine, n, 2)))
End If

n = InStr(strHeader2, "bk")
If n > 0 Then
    rsItems.Fields("pack_bk") = Val(Trim$(Mid$(strLine, n, 2)))
End If

n = InStr(strHeader2, "name")
If n > 0 Then
    rsItems.Fields("name") = Trim$(Mid$(strLine, n))
End If
...
Public Sub ParseShowText(strLine As String, UnitType As String, ReportType As String, LineNumber As Integer)
...
If LineNumber = 1 And Not (rsShowText.BOF And rsShowText.EOF) Then
    ...
    While Not rsShowText.EOF
        If rsShowText.Fields("UnitType") = UnitType And rsShowText.Fields("ReportType") = ReportType Then
            rsShowText.Delete
        End If
        rsShowText.MoveNext
    Wend
...
rsShowText.AddNew
rsShowText.Fields("UnitType") = UnitType
rsShowText.Fields("ReportType") = ReportType
rsShowText.Fields("LineNumber") = LineNumber
rsShowText.Fields("Text") = RTrim$(strLine)
rsShowText.Update
End Sub

Public Sub ParseShowInfrastructure(strLine As String, iLine As Integer)
Dim strID As String
Dim strTokens() As String
...
ParseDumpString strLine, strTokens
If strTokens(0) = "Infrastructure" Or _
   strTokens(0) = "type" Or _
   strTokens(0) = "" Or _
   strTokens(0) = "sector" Then
    Exit Sub
End If
...
If rsBuild.NoMatch Then
    ...
    For iIndex = 0 To rsBuild.Fields.Count - 1
        If rsBuild.Fields(iIndex).Type = dbText Then
...
If strID = "rail" Or _
   strID = "road" Or _
   strID = "defen" Or _
   (frmOptions.bSPAtlantis And (strID = "radar" Or strID = "fort" Or strID = "runwa")) Then
    strParseVariable = "lcms"
    rsBuild.Fields("lcm").Value = CInt(strTokens(2))
    strParseVariable = "hcms"
    rsBuild.Fields("hcm").Value = CInt(strTokens(3))
    strParseVariable = "mobility"
    rsBuild.Fields("stat2").Value = CInt(strTokens(4))
    strParseVariable = "cost"
    rsBuild.Fields("cost").Value = CInt(strTokens(5))
ElseIf frmOptions.bSPAtlantis And strID = "navag" Then
    strParseVariable = "lcms"
    rsBuild.Fields("lcm").Value = CInt(strTokens(1))
    strParseVariable = "hcms"
    rsBuild.Fields("hcm").Value = CInt(strTokens(2))
    strParseVariable = "mobility"
    rsBuild.Fields("stat2").Value = CInt(strTokens(3))
    strParseVariable = "cost"
    rsBuild.Fields("cost").Value = CInt(strTokens(4))
Else
    strParseVariable = "designation cost"
    rsBuild.Fields("stat1").Value = CInt(strTokens(1))
    strParseVariable = "efficiency cost"
    rsBuild.Fields("cost").Value = CInt(strTokens(2))
    strParseVariable = "lcms"
    rsBuild.Fields("lcm").Value = CInt(strTokens(3))
    strParseVariable = "hcms"
    rsBuild.Fields("hcm").Value = CInt(strTokens(4))
End If
...
Public Function ParseWaypoints(strWP As String) As Variant
Dim n As Integer
...
Public Sub ParseShipView(strLine As String)
...
If rsSea.NoMatch Then
    ...
    rsSea.Fields("x") = sx
    rsSea.Fields("y") = sy
    rsSea.Fields("fert") = -1       'negative one mean unknown
    rsSea.Fields("ftimestamp") = GetWinACEUTC
    rsSea.Fields("ocontent") = -1
...
If n > 0 Then
    rsSea.Fields("fert") = CInt(StringBetween(strLine, "[fert:", "]"))
    rsSea.Fields("ftimestamp") = GetWinACEUTC
Else
    n = InStr(strLine, "[oil:")
    If n > 0 Then
        rsSea.Fields("ocontent") = CInt(StringBetween(strLine, "[oil:", "]"))
    End If
    rsSea.Fields("otimestamp") = GetWinACEUTC
End If
...
End Sub
Public Sub ParseAirCombat(strLine As String)
...
Else
    ...
    planeid = CInt(Trim$(Mid$(strLine, n + 1, 5)))
    ...
    If n > 0 Then
        neff = CInt(StringBetween(Mid$(strLine, n), "@", "%"))
    Else
        'get the damage string
        n = InStr(strLine, "/")
        n = InStr(n + 1, strLine, "/")
        If stFirst Then
            nDamage = CInt(Trim$(Mid$(strLine, n - 3, 3)))
        Else
            nDamage = CInt(Trim$(Mid$(strLine, n + 1, 3)))
        End If
    End If
    ...
    If rsAirCombat.NoMatch Then
        rsAirCombat.AddNew
        rsAirCombat.Fields("nation") = stOpponent
        rsAirCombat.Fields("id") = planeid
        rsAirCombat.Fields("type") = strType
        rsAirCombat.Fields("eff") = 100
        rsAirCombat.Fields("numcombats") = 1
    Else
        rsAirCombat.Edit
        rsAirCombat.Fields("numcombats") = rsAirCombat.Fields("numcombats") + 1
        If rsAirCombat.Fields("nextupdate") <> timNextUpdate Then
            rsAirCombat.Fields("eff") = 100
            rsAirCombat.Fields("type") = strType
            rsAirCombat.Fields("numcombats") = 1
        End If
    ...
    rsAirCombat.Fields("nextupdate") = timNextUpdate

    If neff >= 0 Then
        rsAirCombat.Fields("eff") = neff
    Else
        If nDamage > 0 Then
            rsAirCombat.Fields("eff") = rsAirCombat.Fields("eff") - nDamage
            If rsAirCombat.Fields("eff") < 10 Then
                rsAirCombat.Fields("eff") = 0
            End If
        End If
    End If
    'set the plane status
    If rsAirCombat.Fields("eff") >= 40 Then
        rsAirCombat.Fields("status") = "A"
    ElseIf rsAirCombat.Fields("eff") >= 10 Then
        rsAirCombat.Fields("status") = "D"
    Else
        rsAirCombat.Fields("status") = "S"
    End If
...
Public Function GetUnitClass(strID As String, strLine As String) As String
...
If Not (bShip Or bLand Or bPlane) Then
    Load frmAddUnitDes
    frmAddUnitDes.Caption = "Unit Designation " + strID + " not found in Database"
    frmAddUnitDes.Label1.Caption = strLine
    frmAddUnitDes.cmdCancel.Caption = "Ignore"
    frmAddUnitDes.txtID.Text = strID
    frmAddUnitDes.txtDesc.Text = StringBetween(strLine, ")", "(")
    frmAddUnitDes.txtID.Locked = True
    frmAddUnitDes.Show vbModal
    GetUnitClass = frmAddUnitDes.UnitClass
    If Len(GetUnitClass) > 0 Then
        rsBuild.AddNew
        rsBuild.Fields("type") = frmAddUnitDes.UnitClass
        rsBuild.Fields("id") = strID
        rsBuild.Fields("desc") = frmAddUnitDes.txtDesc.Text
        rsBuild.Update
    End If
    Unload frmAddUnitDes
    Exit Function
End If
...
Public Function ParseShellDamage(strLine As String) As Integer
On Error GoTo Empire_Error
'Shell hit sector -3,-1 for 40 damage.
ParseShellDamage = CInt(Trim$(StringBetween(strLine, "for", "damage")))
'Error handling routine
Exit Function
Empire_Error:
...
Public Sub ParseShipOrders(strLine As String, Optional Deityflag As Boolean)
...
If InStr(strLine, "#") = 0 And InStr(strLine, ",") > 0 And Left$(strLine, 7) <> "       " And InStr(strLine, "No ship") = 0 Then
    ...
    Else
        rsShipOrders.AddNew
        rsShipOrders.Fields("id") = id
    End If
    rsShipOrders.Fields("owner") = CInt(owner)
    ...
    rsShipOrders.Fields("curr sector") = strTokens(n)
    n = n + 1
    ...
    If (UBound(strTokens, 1) < n) Or (InStr(strLine, "has a sail path") > 0) Then
        rsShipOrders.Fields("start sector") = "Sail"
    Else
        rsShipOrders.Fields("start sector") = strTokens(n)
        ...
        If (strTokens(n) = "no") Then  ' "no route possible"
            ...
            'like before.
            rsShipOrders.Fields("length") = 999
            rsShipOrders.Fields("eta") = 999
            rsShipOrders.Fields("end sector") = "???"
        Else
            If InStr(strTokens(n), ",") > 0 Then
                If InStr(strTokens(n), "has") > 0 Then '102603 rjk: Server bug no space between End Sector and 'has arrived'.
                    rsShipOrders.Fields("end sector") = Left$(strTokens(n), InStr(strTokens(n), "has") - 1)
                ...
                ElseIf InStr(strTokens(n), "no") > 0 Then '102603 rjk: Server bug no space between End Sector and 'no route possible'.
                    rsShipOrders.Fields("end sector") = Left$(strTokens(n), InStr(strTokens(n), "no") - 1)
                ...
                ElseIf InStr(strTokens(n), "route") > 0 Then '102603 rjk: Server bug no space between End Sector and 'route too long'.
                    rsShipOrders.Fields("end sector") = Left$(strTokens(n), InStr(strTokens(n), "route") - 1)
                    strTokens(n) = Mid$(strTokens(n), InStr(strTokens(n), "route"))
                Else
                    rsShipOrders.Fields("end sector") = strTokens(n)
                    n = n + 1
                End If
            End If
            ...
            ElseIf (strTokens(n) = "route") Then '"route too long".  102603 rjk: Added based on the server code
                ...
                rsShipOrders.Fields("length") = 999
            Else
                rsShipOrders.Fields("length") = strTokens(n)
                rsShipOrders.Fields("eta") = strTokens(n + 1)
            End If
        End If
    End If
...
Public Sub ParseCargoOrders(strLine As String, Optional Deityflag As Boolean)
...
If InStr(strLine, "#") = 0 And InStr(strLine, "[") > 0 Then
    ...
    For n = 1 To 6
        strHold = CStr(n)
        rsShipOrders.Fields("start cargo " + strHold) = vbNullString
        rsShipOrders.Fields("end cargo " + strHold) = vbNullString
        rsShipOrders.Fields("start level " + strHold) = 0
        rsShipOrders.Fields("end level " + strHold) = 0
    Next n
    ...
    If Len(strTemp) > 0 Then
        ...
        For n = LBound(strTokens) To numTokens
            ...
            strLevel = CInt(StringBetween(strTokens(n), ":"))
            
            rsShipOrders.Fields("start cargo " + strHold) = strItem
            rsShipOrders.Fields("start level " + strHold) = strLevel
        Next n
    End If
    ...
    If Len(strTemp) > 0 Then
        ...
        For n = LBound(strTokens) To numTokens
            ...
            rsShipOrders.Fields("end cargo " + strHold) = strItem
            rsShipOrders.Fields("end level " + strHold) = strLevel
        Next n
    End If
...
'Parse the results of a sorder command
Public Sub ParseMissions(strLine As String, strType As String, Optional Deityflag As Boolean)
...
If InStr(strLine, "has no mission") > 0 Then
    ...
    If strTemp = ")" Then
        id = CInt(StringBetween(strLine, "(#", strTemp))
    Else
        id = CInt(StringBetween(strLine, "#", strTemp))
    End If
...
ElseIf InStr(strLine, "on a") > 0 Then
    ...
    Else
        rsMissions.AddNew
        rsMissions.Fields("id") = id
        rsMissions.Fields("type") = strType
    End If
    var = "owner"
    rsMissions.Fields(var) = 0
...
'        numTokens = UBound(strTokens)    8/2003 efj  removed
        ...
        var = "op sector"
        If InStr(strTokens(n), ",") > 0 Then
            rsMissions.Fields(var) = strTokens(n)
            n = n + 1
        Else
            rsMissions.Fields(var) = vbNullString
        End If
        var = "radius"
        rsMissions.Fields(var) = strTokens(n)
        var = "mission"
        ...
        End If
        rsMissions.Fields(var) = strTemp
    Else
        var = "op sector"
        If InStr(strLine, "centered") > 0 Then
            rsMissions.Fields(var) = StringBetween(strLine, "centered on", ", ")
        Else
            rsMissions.Fields(var) = vbNullString
        End If
        ...
        If InStr(strLine, var) > 0 Then
            rsMissions.Fields(var) = CInt(StringBetween(strLine, "radius"))
        Else
            rsMissions.Fields(var) = 0
        End If
        var = "mission"
...
Public Sub ParseAndDeleteUnits(strResponse As String)
